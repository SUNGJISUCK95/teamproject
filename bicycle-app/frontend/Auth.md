...
%% 2. 회원가입
    ...
    Frontend-->>User : 필수정보 미기입 또는 소셜 로그인을 하지 않은 상태에서 아이디 중복확인을 안한 경우 경고창 출력
...
%% 3-1. 아이디 찾기
    ...
    Frontend-->>User : 사용자 정보가 없으면 경고창을 출력.
    ...
    Backend-->>Frontend: DB에서 전달받은 데이터가 없는 경우, 시간이 지난경우 "wrong or late" 문자열 전달.
    Frontend-->>User: "wrong or late" 를 받을 경우, 화면에 경고문구 출력.
...
%% 3-2. 비밀번호 변경하기
    ...
    Frontend-->>User: 비밀번호가 다른 경우, 데이터 제거 후 빈칸 재출력.
...
%% 4. 내 정보 페이지
    ...
    Frontend-->>User: 로컬 스토리지에 아이디가 없는 경우, 잘못된 접근으로 판단하고 홈화면으로 돌려보냄
    ...
    Frontend-->>User : 아이디 값을 수정했으나 중복 확인 하지 않은 경우 경고창 출력
